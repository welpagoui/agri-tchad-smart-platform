...
// INITIALISATION AUTOMATIQUE (Comptes Banque, ONG, Admin)
const initDB = async () => {
    try {
        await pool.query(`
        ...
        console.log("✅ Base de données Agri-Tchad prête avec tous les rôles");
...
app.post('/api/agriculteurs', async (req, res) => {
    ...
    [nom, zone, telephone, culture, Math.floor(Math.random()*100), (12+Math.random()).toFixed(4), (15+Math.random()).toFixed(4)]);
    res.json(r.rows[0]);
});
